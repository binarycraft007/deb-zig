...
pub const ExtractOptions = struct {
    ...
    pub const ModeMode = enum {
        ...
        executable_bit_only,
        /// The mode from the tar file is preserved.
        preserve,
    };
};
    ...
    pub fn next(self: *Iterator) !?File {
        ...
        while (try self.readHeader()) |header| {
            ...
            switch (kind) {
                ...
                .directory, .normal, .normal_alias, .symbolic_link, .hard_link => {
                    file.kind = switch (kind) {
                        .directory => .directory,
                        .normal, .normal_alias => .file,
        ...
        switch (file.kind) {
            .directory => {
                if (file_name.len > 0 and !options.exclude_empty_directories) {
                    makePath(io, dir, file_name) catch |err| {
                    ...
                    if (file.mode != 0 and options.mode_mode == .preserve) {
                        dir.setFilePermissions(io, file_name, .fromMode(@intCast(file.mode)), .{}) catch {};
                    }
                }
            },
            .file => {
            ...
            .hard_link => {
                const link_name = file.link_name;
                createDirAndHardlink(io, dir, link_name, file_name) catch |err| {
                    const d = options.diagnostics orelse return err;
                    try d.errors.append(d.allocator, .{ .unable_to_create_file = .{
                        .code = err,
                        .file_name = try d.allocator.dupe(u8, file_name),
...
fn makePath(io: Io, dir: Io.Dir, sub_path: []const u8) !void {
    if (sub_path.len == 0 or std.mem.eql(u8, sub_path, ".")) return;
...
fn createDirAndFile(io: Io, dir: Io.Dir, file_name: []const u8, permissions: Io.File.Permissions) !Io.File {
    const fs_file = dir.createFile(io, file_name, .{ .exclusive = false, .permissions = permissions }) catch |err| {
        if (err == error.PathAlreadyExists or err == error.IsDir) {
            dir.deleteTree(io, file_name) catch dir.deleteFile(io, file_name) catch {};
            return try dir.createFile(io, file_name, .{ .exclusive = false, .permissions = permissions });
        }
        if (err == error.FileNotFound) {
            if (std.fs.path.dirname(file_name)) |dir_name| {
                try makePath(io, dir, dir_name);
                return dir.createFile(io, file_name, .{ .exclusive = false, .permissions = permissions }) catch |e| {
                    if (e == error.PathAlreadyExists or e == error.IsDir) {
                        dir.deleteTree(io, file_name) catch dir.deleteFile(io, file_name) catch {};
                        return try dir.createFile(io, file_name, .{ .exclusive = false, .permissions = permissions });
                    }
                    return e;
...
fn createDirAndSymlink(io: Io, dir: Io.Dir, link_name: []const u8, file_name: []const u8) !void {
    dir.symLink(io, link_name, file_name, .{}) catch |err| {
        if (err == error.PathAlreadyExists or err == error.IsDir) {
            dir.deleteTree(io, file_name) catch dir.deleteFile(io, file_name) catch {};
            return dir.symLink(io, link_name, file_name, .{});
        }
        if (err == error.FileNotFound) {
            if (std.fs.path.dirname(file_name)) |dir_name| {
                try makePath(io, dir, dir_name);
                return dir.symLink(io, link_name, file_name, .{}) catch |e| {
                    if (e == error.PathAlreadyExists or e == error.IsDir) {
                        dir.deleteTree(io, file_name) catch dir.deleteFile(io, file_name) catch {};
                        return dir.symLink(io, link_name, file_name, .{});
                    }
...
fn createDirAndHardlink(io: Io, dir: Io.Dir, link_name: []const u8, file_name: []const u8) !void {
    var sanitize_buffer: [Io.Dir.max_path_bytes]u8 = undefined;
    const n = sanitizePath(&sanitize_buffer, link_name, 0) catch 0;
    const sanitized_link = if (n > 0) sanitize_buffer[0..n] else link_name;

    dir.hardLink(sanitized_link, dir, file_name, io, .{}) catch |err| {
        if (err == error.PathAlreadyExists or err == error.IsDir) {
            dir.deleteTree(io, file_name) catch dir.deleteFile(io, file_name) catch {};
            return dir.hardLink(sanitized_link, dir, file_name, io, .{});
        }
        if (err == error.FileNotFound) {
            if (std.fs.path.dirname(file_name)) |dir_name| {
                try makePath(io, dir, dir_name);
                return dir.hardLink(sanitized_link, dir, file_name, io, .{}) catch |e| {
                    if (e == error.PathAlreadyExists or e == error.IsDir) {
                        dir.deleteTree(io, file_name) catch dir.deleteFile(io, file_name) catch {};
                        return dir.hardLink(sanitized_link, dir, file_name, io, .{});
                    }
...
fn sanitizePath(buffer: []u8, raw_path: []const u8, strip_components: u32) error{Invalid}!usize {
    const path = std.mem.trimStart(u8, raw_path, "/");
